...
function getModelsForMark(marca) {
  const models = VEHICLES.filter((v) => v.marca === marca).map((v) => v.modelo);
  return [...new Set(models)].sort();
}

// Función para obtener versiones únicas para una marca y modelo
function getVersionsForMarkAndModel(marca, modelo) {
...
function initializeVehicleFields(row) {
  ...
  brandInput.addEventListener("change", function () {
    ...
    if (marca) {
      const models = getModelsForMark(marca);
